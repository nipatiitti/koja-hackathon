  ...
  return (
    <div className="flex flex-col pt-8 px-6 items-center">
      ...
      <label className="fixed top-3 left-28 text-sm text-text-950 rounded-sm bg-secondary gap-2 px-6 py-3 shadow-md">
        <div>
          Total Wattage: <strong>{serverRacks.reduce((sum, rack) => sum + rack.serverWattage, 0)} W</strong>
        </div>
        <div>
          Server Count: <strong>{serverRacks.reduce((sum, rack) => sum + rack.serverAmount, 0)}</strong>
        </div>
      </label>
      <div className="mb-4">
